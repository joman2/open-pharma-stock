...
A universal scanner for Flutter. This fork uses CameraX, ZXing and libdmtx on
Android and retains the upstream AVFoundation implementation on iOS.
...
### Android
This fork uses CameraX for camera frames, ZXing for common barcode formats and
the OpenCV/libdmtx pipeline for difficult DataMatrix reads. It does not include
ML Kit or Google Play Services barcode dependencies on Android.
...
## Vendored changes (open-pharma-stock)

This repository vendors `mobile_scanner` via a local path override in open-pharma-stock to
improve robustness for GS1 DataMatrix, including industrial DPM (dot-peen) codes.

- Vendoring: plugin is copied into `plugins/mobile_scanner` and referenced via `path:` in the app.
- Android flag: `enableFallbackDecoder` is propagated from Dart (controller/start options) to Android
  to toggle fallback decoding inside the analyzer.
- ZXing fallback (single CameraX pipeline): integrated into the analyzer for `DATA_MATRIX`:
  - Uses the Y plane (luminance) and a central ROI.
  - Hints: `TRY_HARDER=true`, `POSSIBLE_FORMATS=[DATA_MATRIX]`.
  - Debug logs: `OPS: ZXING_FALLBACK ...`.
- DPM OpenCV pipeline (experimental, CPU heavy):
  - ROI + CLAHE + morphology + blob detection + homography -> normalizedGray.
  - Debug logs: `OPS: DPM start`, `OPS: DPM ... blobs`, `OPS: DPM homography ok`,
    `OPS: DPM normalized size`, `OPS: DPM fail reason=...`.
- BitMatrix decode (before image decode):
  - Builds a `BitMatrix` from the module grid.
  - Quiet zone q=2 and q=3, with a sweep around `modulesGuess`.
  - Debug logs: `OPS: DPM BitMatrix try/OK/FAIL`.
- Soft per-module decode (experimental):
  - Classifies STRONG vs AMBIGUOUS modules and tries a limited set of flipped candidates.
  - Debug logs: `OPS: DPM soft start/candidates/OK/FAIL_ALL`.
- libdmtx via JNI/NDK (experimental):
  - `CMakeLists.txt`, `dmtx_jni.cpp`, `DmtxNative.kt`, and vendored libdmtx sources.
  - Runs immediately after `normalizedGray`, short-circuits on success.
  - Debug logs: `OPS: DPM DMTX try/OK/FAIL`.
  - ABIs: `arm64-v8a`, `armeabi-v7a`.
- DMTX multi-frame stacking (experimental):
  - Ring buffer of `normalizedGray` frames and stacked median decode before any ZXing paths.
  - Debug logs: `OPS: DPM STACK ...` and `OPS: DPM DMTX_STACK ...`.
  - Intended for "pocket use"; may add ~0.3-1s latency.

### Debugging with ADB logcat

Example (Windows):
```
adb logcat | findstr /i "OPS: DPM DMTX DMTX_STACK BitMatrix soft ZXING_FALLBACK"
```
