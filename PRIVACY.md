...
Camera access is used to scan barcodes. Frames are processed on the device by
FLOSS barcode libraries and are not uploaded by the scanner.
...
The option can be disabled at any time. Imported local CSV catalogue data takes
priority over remotely obtained metadata.

## Files and sharing

Exports and backups are created only after a user action and may contain
sensitive operational data. Their storage and sharing are controlled by the
user and the selected Android file provider.

## Public support

Do not include real pharmacy data in public issues, screenshots or logs. Use
synthetic or redacted examples. For a sensitive report, open a minimal issue
requesting a private contact route without including the sensitive material.
